...
export const getNumberedArray = (arr: string[]): T[] => arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: T[]): string[] => arr.map(({ value, number }) => `${value}_${number}`);
...
export type CustomerMap = Record<number, { name: string; age: number; isSubscribed: boolean }>;

export const transformCustomers = (customers: Array<Customer>): CustomerMap => {
